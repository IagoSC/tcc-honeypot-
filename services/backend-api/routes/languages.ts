...
export function getLanguageList () {
  return (req: Request, res: Response, next: NextFunction) => {
    ...
    fs.readFile(path.join(I18N_DIR, 'en.json'), 'utf-8', (err, content) => {
      if (err != null) {
        next(new Error(`Unable to retrieve en.json language file: ${err.message}`))
        return
      }
      enContent = JSON.parse(content)
      fs.readdir(I18N_DIR, (err, languageFiles) => {
        if (err != null) {
          next(new Error(`Unable to read i18n directory: ${err.message}`))
          return
        }
        const jsonFiles = languageFiles.filter(f => f.endsWith('.json'))
        jsonFiles.forEach((fileName) => {
          // eslint-disable-next-line @typescript-eslint/no-misused-promises
          fs.readFile(path.join(I18N_DIR, fileName), 'utf-8', async (err, content) => {
            if (err != null) {
